...
export function bindTestStubs(stubs?) {
  ...
  let allStubs = {
    ...
    "FaucetWorkers.createChildProcess": sinon.stub(FaucetWorkers.prototype, "createChildProcess").callsFake((classKey) => {
      ...
      (worker.childProcess as any).send = (
        message: any,
        handleOrCallback?: any,
        optionsOrCallback?: any,
        callback?: (error: Error | null) => void,
      ) => {
        let handle = typeof handleOrCallback === "function" ? undefined : handleOrCallback;
        let sendCallback = typeof handleOrCallback === "function"
          ? handleOrCallback
        ...
        if(handle) {
          setTimeout(() => {
            (channel.port2.onmessage as any)({data: message, objs: handle} as any);
          }, 1);
        } else {
          channel.port1.postMessage(message);
        }
        queueMicrotask(() => sendCallback?.(null));
        return true;
      };
      setTimeout(() => {
        FaucetWorkers.loadWorkerClass(classKey, channel.port2 as any);
