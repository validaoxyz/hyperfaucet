...
describe("Faucet module: ethinfo", () => {
  ...
  it("drains on disable without disposing the shared payout provider", async () => {
    faucetConfig.modules["ethinfo"] = {
      enabled: true,
      requestTimeout: 1000,
    ...
    const pendingBalance = new PromiseDfd<bigint>();
    sinon.stub(walletManager, "getWalletBalance").returns(pendingBalance.promise);
    const dispose = sinon.spy(walletManager, "dispose");
    await ServiceManager.GetService(ModuleManager).initialize();
    const module = ServiceManager.GetService(ModuleManager).getModule<EthInfoModule>("ethinfo");
    const session = {
      getSessionData: () => [],
      getTargetAddr: () => "0x0000000000000000000000000000000000001337",
    };

    const oldLookup = (module as any).processSessionStart(session, {});
    await Promise.resolve();
    let stopSettled = false;
    const stop = module.disableModule().then(() => {
      stopSettled = true;
    });
    ...
    expect((oldError as FaucetError).getCode()).to.equal("ETHINFO_CHECK");
    expect(stopSettled).to.equal(false, "module stopped before the shared RPC drained");
    expect(dispose.called).to.equal(false, "module disposed the shared payout provider");

    pendingBalance.resolve(1000n);
    await stop;
    expect(dispose.called).to.equal(false, "module disposed the shared payout provider after drain");
  });
