...
describe("Faucet module: authenticatoor (module)", () => {
  ...
  it("Module reload reinitializes verifier", async () => {
    ...
    let mod = ServiceManager.GetService(ModuleManager).getModule<any>("authenticatoor");
    mod.setModuleConfig({
      ...(faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig),
      expectedAudience: "new.aud",
    });
    expect(mod.verifier).to.not.equal(null, "verifier should be re-created");
  });

  it("applies the policy paired with the verifier when configuration reloads during verification", async () => {
    let resolveVerification: (claims: IAuthenticatoorClaims) => void;
    verifyStub.callsFake(() => new Promise<IAuthenticatoorClaims>((resolve) => {
      resolveVerification = resolve;
    }));
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).grants = [{
      limitCount: 0,
      limitAmount: 0,
      duration: 3600,
      overrideMaxDrop: 111,
    ...
    let sessionPromise = ServiceManager.GetService(SessionManager).createSession("::ffff:8.8.8.8", {
      addr: "0x0000000000000000000000000000000000001337",
      authToken: tokenFor("alice@example.com"),
    });
    await awaitSleepPromise(100, () => verifyStub.calledOnce);
    expect(verifyStub.calledOnce).to.equal(true);

    let mod = moduleManager.getModule<any>("authenticatoor");
    mod.setModuleConfig({
      ...(faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig),
      grants: [{
        limitCount: 0,
    ...
    expect(session.getDropAmount()).to.equal(111n);
    expect(session.getSessionData("overrideMaxDropAmount")).to.equal("111");
  });
  ...
  it("Valid token: stores auth data and applies grant perks", async () => {
    ...
    expect(data.subject).to.equal("alice@example.com", "subject mismatch");
  ...
  it("Token without issuer/subject is rejected", async () => {
  ...
  it("Token with subject but no email keeps the stable subject and deduplicates admission aliases", async () => {
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).concurrencyLimit = 2;
    ...
    let data = testSession.getSessionData<any>("authenticatoor.data");
    expect(data.subject).to.equal("subject-id-123", "subject mismatch");
    expect(data.email).to.equal("", "email should be empty when not in token");

    let secondSession = await ServiceManager.GetService(SessionManager).createSession("::ffff:8.8.4.4", {
      addr: "0x0000000000000000000000000000000000001338",
      authToken: "anything",
    });
    expect(secondSession.getSessionStatus()).to.equal(FaucetSessionStatus.RUNNING, "one prior session should count once across aliases");

    let error: FaucetError | null = null;
    try {
      await ServiceManager.GetService(SessionManager).createSession("::ffff:1.1.1.1", {
        addr: "0x0000000000000000000000000000000000001339",
        authToken: "anything",
      });
    } catch(ex) {
      error = ex;
  ...
  it("admits only the first concurrent start for one authenticated user", async () => {
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).concurrencyLimit = 1;
    await ServiceManager.GetService(ModuleManager).initialize();
    ServiceManager.GetService(ModuleManager).addActionHook(null, ModuleHookAction.SessionStart, 100, "blocker", (session: FaucetSession) => {
      session.addBlockingTask("test", "test", 10);
    ...
    expect(rejected[0].reason.getCode()).to.equal("AUTHENTICATOOR_CONCURRENCY_LIMIT");
  });

  it("reserves grant claim ceilings during concurrent amount checks", async () => {
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).grants.push({
      duration: 3600,
      limitAmount: 2000000000000000000,
      limitCount: 0,
      overrideMaxDrop: 2000000000000000000,
      required: true,
    });
    ...
    expect(fulfilled.length).to.equal(1);
    expect(fulfilled[0].value.getSessionData("overrideMaxDropAmount")).to.equal("2000000000000000000");
    expect(rejected.length).to.equal(1);
    expect(rejected[0].reason).to.be.instanceOf(FaucetError);
    expect(rejected[0].reason.getCode()).to.equal("AUTHENTICATOOR_LIMIT");
  });

  it("restores authenticated-user reservations before admitting new traffic", async () => {
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).concurrencyLimit = 1;
    await ServiceManager.GetService(ModuleManager).initialize();
    let now = Math.floor(Date.now() / 1000);
    await ServiceManager.GetService(FaucetDatabase).updateSession({
      sessionId: "4e63566e-e482-46f3-bb91-da11f511c002",
  ...
  it("restores legacy userId reservations before admitting a new same-email subject", async () => {
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).concurrencyLimit = 1;
    verifyStub.callsFake(async () => ({
  ...
  it("SessionComplete persists the stable subject rather than the legacy email", async () => {
    verifyStub.callsFake(async () => ({
      sub: "stable-subject-123",
      email: "alice@example.com",
      iss: AUTH_URL,
    ...
    let rows = await db.getPrincipalSessions({issuer: AUTH_URL, subject: "stable-subject-123"}, ["alice@example.com"], 3600, true);
    ...
    let storedIdentity = await ServiceManager.GetService(FaucetDatabase).getDatabase().get(
      "SELECT UserId,Issuer FROM AuthenticatoorSessions WHERE SessionId = ?",
      [testSession.getSessionId()],
    ) as {UserId: string, Issuer: string};
    expect(storedIdentity.UserId).to.equal("stable-subject-123", "new row stored the legacy email instead of subject");
    expect(storedIdentity.Issuer).to.equal(AUTH_URL, "new row issuer mismatch");
  });

  it("counts same-issuer historical email UserId rows without crossing issuer identity domains", async () => {
    (faucetConfig.modules["authenticatoor"] as IAuthenticatoorConfig).grants.push({
      duration: 3600,
      limitAmount: 0,
      limitCount: 1,
    ...
    let legacySessionId = "4e63566e-e482-46f3-bb91-da11f511c004";
    await faucetDb.updateSession({
    ...
    await faucetDb.getDatabase().run(
      "INSERT INTO AuthenticatoorSessions (SessionId,UserId,Issuer,StartTime) VALUES (?,?,?,?)",
      [legacySessionId, "alice@example.com", AUTH_URL, now],
    );
    ...
    let mod = ServiceManager.GetService(ModuleManager).getModule<any>("authenticatoor");
    let db: AuthenticatoorDB = mod.authDb;
    let crossIssuerRows = await db.getPrincipalSessions(
      {issuer: "https://rotated-auth.test.local", subject: "stable-subject-123"},
      ["alice@example.com"],
      3600,
      true,
    );
    expect(crossIssuerRows.length).to.equal(0, "legacy email alias crossed the issuer identity boundary");
  });
  ...
  it("SessionComplete is a no-op when no auth data stored", async () => {
    ...
    let mod = ServiceManager.GetService(ModuleManager).getModule<any>("authenticatoor");
    let db: AuthenticatoorDB = mod.authDb;
    let rows = await db.getPrincipalSessions({issuer: AUTH_URL, subject: "alice@example.com"}, ["alice@example.com"], 3600, true);
  ...
  it("Database cleanStore removes orphaned rows", async () => {
    ...
    let faucetDb = ServiceManager.GetService(FaucetDatabase);
    await faucetDb.getDatabase().run(
      "UPDATE AuthenticatoorSessions SET StartTime = ? WHERE SessionId = ?",
      [now - faucetConfig.sessionCleanup - 10, testSession.getSessionId()],
    );
    await faucetDb.getDatabase().run(
    ...
    let db: AuthenticatoorDB = mod.authDb;
    await db.cleanStore();
    let rows = await db.getPrincipalSessions({issuer: AUTH_URL, subject: "alice@example.com"}, ["alice@example.com"], 99999999, true);
