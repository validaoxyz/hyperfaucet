...
import type { PoWParams } from "../common/FaucetConfig";
import type { PoWSession } from "./PoWSession";
import type { FaucetTime } from '../common/FaucetTime';
import { PoWHashAlgo } from '../types/PoWMinerSrc';
import type { PoWMinerWorkerSrc } from '../types/PoWMinerSrc';
...
interface IPoWMinerWorker {
  ...
  verificationQueue: IPoWMinerVerification[];
}
...
export class PoWMiner extends TypedEmitter<PoWMinerEvents> {
  ...
  private verifyWorkers = new Map<PoWHashAlgo, IPoWMinerWorker>();
  ...
  public stopMiner() {
    this.stopAllWorker();
    for(let worker of this.verifyWorkers.values())
  ...
  private async startStopWorkers() {
    ...
    while(this.workers.length < this.settings.workerCount) {
      // start worker
      this.workers.push(this.startWorker(this.options.powParams.a, false));
    }
    this.getVerifyWorker(this.options.powParams.a);
  }

  private startWorker(algo: PoWHashAlgo, verifyWorker: boolean): IPoWMinerWorker {
    let worker: IPoWMinerWorker = {
      id: verifyWorker ? -1 : this.workers.length,
      algo,
  ...
  private terminateWorker(worker: IPoWMinerWorker) {
    worker.active = false;
  ...
  private async onWorkerPreInit(worker: IPoWMinerWorker) {
    ...
    let workerCode = await this.getWorkerCode(worker.algo);
    if(!worker.active)
      return;
    worker.worker.postMessage({
      action: "workerCode",
  ...
  private onWorkerInit(worker: IPoWMinerWorker) {
    ...
    if(worker.verifyWorker) {
      for(let verification of worker.verificationQueue)
        this.sendVerification(worker, verification);
      worker.verificationQueue = [];
    }
    else {
  ...
  public processVerification(verification: IPoWMinerVerification) {
    let algo = verification?.policy?.params?.a;
    if(!this.isPoWHashAlgo(algo))
      return;

    let verifyWorker = this.getVerifyWorker(algo);
    if(verifyWorker.ready)
      this.sendVerification(verifyWorker, verification);
    else
      verifyWorker.verificationQueue.push(verification);
  }

  private getVerifyWorker(algo: PoWHashAlgo): IPoWMinerWorker {
    let worker = this.verifyWorkers.get(algo);
    if(worker)
      return worker;

    worker = this.startWorker(algo, true);
    this.verifyWorkers.set(algo, worker);
    return worker;
  }

  private sendVerification(worker: IPoWMinerWorker, verification: IPoWMinerVerification): void {
    worker.worker.postMessage({
      action: "verify",
      data: verification
    });
  ...
  private isPoWHashAlgo(value: unknown): value is PoWHashAlgo {
    switch(value) {
