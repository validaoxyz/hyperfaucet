...
export class PoWSession extends TypedEmitter<PoWSessionEvents> {
  ...
  public resumeSession() {
    ...
    this.options.client.on("error", (message) => {
      this.options.showNotification("error", "WS Error: [" + message.data.code + "] " + message.data.message, true);
  ...
  private processVerifyQueue() {
    let queuedResults = this.verifyResultQueue.splice(0);
    queuedResults.forEach((result) => this._submitVerifyResult(result));
  }
  ...
  private _submitVerifyResult(result: IPoWMinerVerificationResult) {
    this.options.client.sendRequest("verifyResult", result).catch((err) => {
      this.options.showNotification("error", "Verification error: [" + err.code + "] " + err.message, true);
    });
  }
