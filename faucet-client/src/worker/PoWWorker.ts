...
export class PoWWorker {
  ...
  private onCtrlVerify(share: any) {
    ...
    let verificationParams = this.getWorkerParams(share.policy.params, share.policy.difficulty);

    let isValid = true;
    if(this.options.configFn)
      this.options.configFn(preimg, verificationParams.params);
    let hash = this.checkHash(share.nonce, preimg, verificationParams);
    if(!hash) {
      isValid = false;
    } else if(verificationParams.params.a === PoWHashAlgo.NICKMINER && hash !== share.data) {
      isValid = false;
    }
  ...
  private checkHash(nonce: number, preimg: string, workerParams = this.powParams): string {
    ...
    this.statsCount++;
    let hashHex = this.options.hashFn(nonceHex, preimg, workerParams.params);

    if(workerParams.params.a === PoWHashAlgo.NICKMINER) {
      ...
      return (difficulty >= workerParams.difficulty) ? hashHex : null;
    } else {
      let startOfHash = hashHex.substring(0, workerParams.dmask.length);
      return (startOfHash < workerParams.dmask) ? hashHex : null;
    }
  }
