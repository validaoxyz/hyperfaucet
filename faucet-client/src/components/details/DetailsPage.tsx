...
import { IClientSessionDetails, IFaucetSessionStatus } from '../../common/FaucetSession';
...
export interface IDetailsPageState {
  ...
  sessionDetails: IClientSessionDetails | null;
...
export class DetailsPage extends React.PureComponent<IDetailsPageProps, IDetailsPageState> {
  ...
  private async refreshSessionStatus() {
    ...
    try {
      const response = await this.props.pageContext.faucetApi.getSessionStatus(this.props.sessionId, true);
      this.lastStatusPoll = (new Date()).getTime();
      const { details: sessionDetails, ...sessionStatus } = response;
      this.setState({
        loadingStatus: false,
        sessionStatus,
  ...
  private renderTxStatusBadge(): React.ReactElement {
    switch(this.state.sessionStatus.claimStatus) {
      ...
      case "prepared":
        return (<span className="badge bg-primary">Submitting</span>);
      case "processing":
      case "pending":
      ...
      case "reverted":
        return (<span className="badge bg-danger">Reverted</span>);
      case "failed":
        return (<span className="badge bg-danger">Failed</span>);
      default:
