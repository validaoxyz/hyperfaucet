...
import { IClientClaimStatus, IClientSessionStatus, IFaucetRpcEndpointStatus, IFaucetStatusDynamicOutflowStatus, IFaucetStatusGeneralStatus, IFaucetStatusOutflowStatus, IFaucetStatusRefillStatus } from '../../types/FaucetStatus';
...
export interface IFaucetStatusPageState {
  ...
  dynamicOutflowStatus: IFaucetStatusDynamicOutflowStatus;
...
export class FaucetStatusPage extends React.PureComponent<IFaucetStatusPageProps, IFaucetStatusPageState> {
  ...
  private refreshFaucetStatus() {
    ...
    void this.props.pageContext.faucetApi.getFaucetStatus().then((faucetStatus) => {
      ...
      this.setState({
        ...
        dynamicOutflowStatus: faucetStatus.dynamicOutflow,
    ...
    return (
      <div className='container grid faucet-status'>
        <div className='row'>
          ...
          <div className='col'>
            <button type="button" className="btn status-refresh" onClick={() => this.refreshFaucetStatus()} disabled={this.state.refreshing}>Refresh</button>
          </div>
        </div>
        <div className='row'>
    ...
    return (
      <div className="container status-general">
        <div className="row">
          ...
          <div className="col-xl-3 col-lg-4 col-6">
            <div className="status-block">
              <div className="status-prop">
                ...
                <span className="status-value">{toReadableAmount(BigInt(this.state.outflowStatus.amount), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)} / {renderTimespan(this.state.outflowStatus.duration, 2)}</span>
              </div>
              <div className="status-prop">
                <span className="status-title">Outflow balance:</span>
                <span className="status-value">{BigInt(this.state.outflowStatus.balance) > 0n ? "+" : "-"} {toReadableAmount(BigInt(this.state.outflowStatus.balance) < 0n ? -BigInt(this.state.outflowStatus.balance) : BigInt(this.state.outflowStatus.balance), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)}</span>
              </div>
              <div className="status-prop">
                <span className="status-title">Balance limits:</span>
                <span className="status-value">{toReadableAmount(BigInt(this.state.outflowStatus.lowerLimit), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)} / {toReadableAmount(BigInt(this.state.outflowStatus.upperLimit), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)}</span>
              </div>
              <div className="status-prop">
          ...
          : null}
          {this.state.dynamicOutflowStatus ?
          <div className="col-xl-3 col-lg-4 col-6">
            <div className="status-block">
              <div className="status-prop">
                <span className="status-title">Drain rate:</span>
                <span className="status-value">{toReadableAmount(parseFloat(this.state.dynamicOutflowStatus.rate || "0") * 86400, this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)} / day</span>
              </div>
              <div className="status-prop">
                <span className="status-title">Drain target:</span>
                <span className="status-value">{renderTimespan(this.state.dynamicOutflowStatus.targetDrainTime, 2)}</span>
              </div>
              <div className="status-prop">
                <span className="status-title">Drain budget:</span>
                <span className="status-value">{parseFloat(this.state.dynamicOutflowStatus.budget) >= 0 ? "+" : "-"} {toReadableAmount(Math.abs(parseFloat(this.state.dynamicOutflowStatus.budget || "0")), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)}</span>
              </div>
              <div className="status-prop">
                <span className="status-title">Drain restriction:</span>
                <span className="status-value">{Math.round(this.state.dynamicOutflowStatus.restriction * 100 * 10) / 10} %</span>
              </div>
            </div>
          </div>
          : null}
  ...
  private renderActiveClaimRow(claim: IClientClaimStatus): React.ReactElement {
    ...
    switch(claim.status) {
      case "queue":
        ...
        break;
      case "prepared":
        claimStatus = <span className="badge bg-primary">Submitting</span>;
        break;
      case "pending":
      ...
      case "failed":
        ...
        >
          <span className="badge bg-danger">{claim.status === "reverted" ? "Reverted" : "Failed"}</span>
        </OverlayTrigger>;
        break;
      default:
