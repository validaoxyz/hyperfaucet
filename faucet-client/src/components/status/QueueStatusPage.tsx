...
export class QueueStatusPage extends React.PureComponent<IQueueStatusPageProps, IQueueStatusPageState> {
  ...
  private refreshQueueStatus() {
    ...
    void this.props.pageContext.faucetApi.getQueueStatus().then((queueStatus) => {
    ...
    return (
      <div className='container grid faucet-status'>
        <div className='row'>
          ...
          <div className='col'>
            <button type="button" className="btn status-refresh" onClick={() => this.refreshQueueStatus()} disabled={this.state.refreshing}>Refresh</button>
          </div>
        </div>
        <div className='row'>
