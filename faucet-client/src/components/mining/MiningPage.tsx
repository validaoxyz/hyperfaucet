...
export class MiningPage extends React.PureComponent<IMiningPageProps, IMiningPageState> {
  ...
  constructor(props: IMiningPageProps) {
    ...
    this.eventListeners = {
      "clientOpen": {
        ...
        event: "open",
        listener: () => {
          void this.props.pageContext.refreshConfig().catch(() => undefined);
  ...
  public componentWillUnmount() {
    ...
    if(this.connectionAlertId !== null) {
      this.props.pageContext.hideStatusAlert(this.connectionAlertId);
      this.connectionAlertId = null;
    }
  }
      ...
      <div className='page-mining'>
        <div className="pow-status-container">
          <PoWMinerStatus 
            ...
            stakeBoostInfo={this.faucetSession.getModuleState("hyperliquid-stake")}
            openStakeInfo={() => this.onOpenStakeClick()}
          />
          <button
            className="btn btn-success stop-action"
  ...
  private async onStopMiningClick(force?: boolean) {
    if(!this.state.isClaimable && this.powSession.getBalance() > 0n && !force) {
      this.props.pageContext.showDialog({
        ...
        body: (
          <div>
            {toReadableAmount(this.powSession.getBalance(), this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)} mined so far, under the {toReadableAmount(this.props.faucetConfig.minClaim, this.props.faucetConfig.faucetCoinDecimals, this.props.faucetConfig.faucetCoinSymbol)} minimum. Stop now and it's gone.
          </div>
        ),
        closeButton: {
          caption: "Keep mining",
  ...
  private async onOpenStakeClick() {
    this.props.pageContext.showDialog({
      title: "Stake boost",
      body: (
        <StakeInfo
          ...
          sessionStartTime={this.faucetSession.getStartTime()}
          targetAddr={this.faucetSession.getTargetAddr()}
          refreshFn={(boost) => {
            let moduleState = this.faucetSession.getModuleState("hyperliquid-stake") || {};
            this.faucetSession.setModuleState("hyperliquid-stake", {...moduleState, boost: boost});
            this.setState({
  ...
  private processSessionStatusRedirects(sessionInfo: IFaucetSessionInfo) {
    ...
    else if(sessionInfo.status === "failed" && (sessionInfo as any).failedCode === "AMOUNT_TOO_LOW") {
      // a deliberate stop below the claim minimum, not an error: no failure
      // page, just back to the start
      FaucetSession.persistSessionInfo(null);
      this.props.pageContext.showNotification("info", "Session ended. Nothing to claim.", true, 6000);
      this.props.navigateFn("/");
    }
