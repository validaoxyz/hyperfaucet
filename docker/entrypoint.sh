...
if [ "${FAUCET_ROLE:-combined}" = "app" ]; then
  export FAUCET_SERVER_PORT="${FAUCET_SERVER_PORT:-8082}"
  export FAUCET_HTTP_PROXY_OFFSET="${FAUCET_HTTP_PROXY_OFFSET:-1}"
  exec node --no-deprecation /app/bundle/powfaucet.cjs "$@"
fi

if [ "${FAUCET_ROLE:-combined}" != "combined" ]; then
  echo "unsupported FAUCET_ROLE: ${FAUCET_ROLE}" >&2
  exit 64
fi
