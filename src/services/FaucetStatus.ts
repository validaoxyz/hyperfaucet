...
export class FaucetStatus {
  ...
  public getFaucetStatus(clientVersion?: string, session?: FaucetSession): {status: IFaucetStatus[], hash: string} {
    ...
    let addStatus = (status: IFaucetStatusEntry) => {
      ...
      const behaviorKey = status.blocksSessionStart ? ":blocks-session-start" : "";
      statusHash.update((status.key || "*") + ":" + text + behaviorKey + "\n");
      statusList.push({
        level: status.level,
        prio: status.prio,
        text: text,
        ishtml: status.ishtml,
        ...(status.blocksSessionStart ? {blocksSessionStart: true} : {}),
      });
    };
