...
export interface IConfigSchema {
  ...
  buildSeoIndex: boolean; // build index.seo.html with configured metadata and versioned client assets
  ...
  faucetDonationAddress: string; // donation/refill address shown on the info page (separate from the hot wallet)
  ...
  faucetSecret: string; // random master secret for server-issued authentication tokens
  pseudonymKey: string; // independent random key used to pseudonymize public session and IP identifiers
  statusAdminToken: string | null; // bearer token for unmasked operator status; null disables the operator endpoint
